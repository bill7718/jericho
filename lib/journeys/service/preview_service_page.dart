    ...
    if (itemIndex < contents.length) {
      switch (contents[itemIndex]['type']) {
        ...
        case liturgy:          
          ...
          widgets.add(TextSplitter(spans: spans, callback: (splits) {
            setState(() {
              widgets.removeLast();
        ...
        default: 
          widget.eventHandler.handleException(context, Exception('invalid service item type ${contents}'), StackTrace.empty);
      }
    }
    
    
    return SimpleGridView(children: widgets);
  }
